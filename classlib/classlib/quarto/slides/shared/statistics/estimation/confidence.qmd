...
ax1.plot(x, pdf, linewidth=4)

left = mu - z*se
right = mu + z*se

mask = (x >= left) & (x <= right)
ax1.fill_between(x[mask], 0, pdf[mask], alpha=0.3)

ax1.set_yticks([])
ax1.set_xticks([])

for xpos in [left, mu, right]:
    ax1.plot([xpos, xpos], [0, -0.03],
             transform=ax1.get_xaxis_transform(),
             clip_on=False, linewidth=3, color="tab:blue")

ax1.text(mu, -0.10, r"$\mu$",
         transform=ax1.get_xaxis_transform(),
...
ax.plot(x_grid, y_grid, linewidth=4)
ax.fill_between(x_grid, 0, y_grid, alpha=0.25)

# Small tick at μ (no vertical line)
ax.plot(
    [mu, mu],
    [0, -0.05],
...
ax.set_title(
    rf"Sampling distribution of $\overline{{X}}$ for $X_i \sim \mathrm{{Exp}}(\mu)$, $n={n}$",
    fontsize=24
)

ax.set_yticks([])
ax.set_xticks([])        # remove numeric x-axis ticks
