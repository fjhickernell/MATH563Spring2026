...
It may be easier to work with the [log-likelihood]{.alert} $\ell(\vtheta) := \log(L(\vtheta))$ since the logarithm is a monotone transformation, so
$$
\vTheta_{\MLE}  = \Argmax{\vtheta} \ell(\vtheta | X_1, \ldots, X_n) 
$$
