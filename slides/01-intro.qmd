...
<div class="fragment" data-fragment-index="1" style="width:100%; margin:0;" markdown="1">
  ...
  \text{BUT } 
  F_{X,Y}(-0.8,- 0.8) &= 0 \ne [1 - \cos^{-1}(x)/\pi ]^2 = F_X(0.8) F_Y(0.8)
  \end{align*}
</div>
...
- [Binomial]{.alert} for _fixed_ $n$ and $0 < p < 1$: $\quad \displaystyle \varrho(x) = \binom{n}{x} p^x (1-p)^{n-x} = \bbone(x \in \{0, \ldots, n\}) \binom{n}{x} \exp\biggl(\log\biggl(\frac{p}{1-p} \biggr ) x \biggr)$

- [Exponential]{.alert}: $\quad \varrho(x) = \bbone(x \ge 0) \exp(-\lambda x)$
...
[Zero-inflated exponential distribution]{.alert} — There is a taxi waiting for you with probabilit $p$, but if there is none, then you have to wait time $1/\lambda$ on average
$$
\begin{align*}
F(x) &= \begin{cases} 
0, &-\infty < x < 0 \\
...
\end{cases}
\\
\mu & = \Ex[X] = \int_{-\infty}^{\infty} x \dif F(x) \quad [\text{Lebesgue-Stieltjes intgral}]{.alert}
\end{align*}
$$
