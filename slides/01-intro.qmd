...
- But if there is none, then you have to wait time $1/\lambda$ on average [Lebesgue–Stieltjes integral](https://en.wikipedia.org/wiki/Lebesgue–Stieltjes_integration)
\begin{align*}
F(x) &= \begin{cases} 
0, &-\infty < x < 0 \\
...
\end{cases}
\\
\mu & = \Ex(X) = \int_{-\infty}^{\infty} x \, \dif F(x) \quad \href{https://en.wikipedia.org/wiki/Lebesgue–Stieltjes_integration}
     {\text{(Lebesgue–Stieltjes integral)}}\\
