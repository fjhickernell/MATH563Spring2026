...
[Zero-inflated exponential distribution]{.alert} — A taxi waiting for you with probability $p$, but if there is none, then you have to wait time $1/\lambda$ on average
\begin{align*}
F(x) &= \begin{cases} 
0, &-\infty < x < 0 \\
...
\end{cases}
\\
\mu & = \Ex(X) = \int_{-\infty}^{\infty} x \, \dif F(x) \quad \class{alert}{\text{(Lebesgue-Stieltjes intgral)}} \\
