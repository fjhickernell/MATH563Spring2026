...
- [Quantile function]{.alert} $Q$ of a random variable $X$ plays the role of the inverse of the CDF
  - $Q(p) : = \inf\{x \in \reals : F(x) \ge p\}$

- [Probability mass function (PMF)]{.alert} $\varrho$ of discrete random variable $X$ 
  - $\varrho(x):= \Prob(X = x)$

- [Probability density function (PDF)]{.alert} $\varrho$ of continuous random variable $X$ 
  - $\varrho(x):= F'(x)$
...
- [Median]{.alert} of $X$ is $\med(X) : = Q(1/2)$
...
- $X, X_1, X_2, \ldots \IIDsim \Norm(0,1)$ satisfies $X_n \xrightarrow{d} X$ but not $X_n \ \not\!\xrightarrow{\Prob} X$

- $X=0$ and $\Prob(X_n =x ) = \begin{cases} 1/n, & x=1 \\ 1-1/n, & x=0 \end{cases} \;$  satisfies
$$
\Prob(\lvert X_n - X \rvert > \varepsilon ) = \Prob(X_n =1) = 1/n \to 0 \text{ as } n \to \infty \text{ so } X_n \xrightarrow{\Prob} X
$$
  But $\Prob (X_n =1 \text{ infinitely often}) = 1$ since
$$
\sum_{n=1}^{\infty} \Prob(X_n=1) = \sum_{n=1}^{\infty} \frac 1n = \infty
$$
so  $X_n \ \, \not\!\!\xrightarrow{\text{a.s.}} X$
