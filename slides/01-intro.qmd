...
- [Binomial]{.alert} (free-throw shooting) $\Bin(n,p)$
$$
\varrho(x) : = \Prob(X=x) = \binom{n}{x} p^x (1-p)^{n-x}, \ x =0, \ldots, n; \qquad \Ex(X) = np, \quad \var(X) = np(1-p)
$$

- [Exponential]{.alert} (taxi waiting) $\Exp(\lambda)$
$$
F(x): = 1 - \exp(-\lambda x),\ \varrho(x) : = \exp(-\lambda x), \ x \ge 0; \qquad \Ex(X) = 1/\lambda, \quad \var(X) = 1/\lambda^2
$$